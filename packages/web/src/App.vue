<template>
  <div
    class="h-screen w-screen flex flex-col bg-explorer-bg text-explorer-text overflow-hidden select-none"
  >
    <!-- Header / Navbar Utama -->
    <header
      class="h-16 border-b border-explorer-border flex items-center justify-between px-6 bg-explorer-sidebar/50 backdrop-blur-md z-10"
    >
    ...
    <div class="flex-1 flex min-h-0">
      ...
      <aside
        class="w-72 border-r border-explorer-border bg-explorer-sidebar flex flex-col p-4 min-w-[240px] max-w-sm"
      >
        <h2 class="text-xs uppercase font-bold text-explorer-muted tracking-wider mb-3 pl-2">
          Navigasi Folder
        </h2>
        ...
        <div class="flex-1 overflow-y-auto">
          <div
            v-if="isTreeLoading"
            class="flex items-center justify-center py-8 text-explorer-muted text-xs space-x-2"
          >
            <div
      ...
      <main class="flex-1 flex flex-col bg-explorer-bg p-6 min-w-0">
        ...
        <div
          class="flex-1 bg-explorer-sidebar/20 border border-explorer-border/30 rounded-xl p-5 shadow-inner min-h-0 flex flex-col"
        >
          <div
            v-if="!selectedFolderId && !isSearching"
            class="flex-1 flex flex-col items-center justify-center text-explorer-muted space-y-4"
          >
