...
export async function seedFolders(db: DbType) {
  ...
  const [docFolder, picFolder, musicFolder, downloadFolder] = await db.insert(folders).values([
  ...
  const [workFolder, personalFolder, archiveFolder] = await db.insert(folders).values([
