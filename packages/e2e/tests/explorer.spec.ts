...
test.describe('File Explorer App', () => {
  ...
  test('should display initial load state with root folders in sidebar', async ({ page }) => {
    ...
    await expect(page.locator('p.text-explorer-muted')).toHaveText('Sistem Manajemen Folder Hirarkis');
    ...
    await expect(page.locator('text=Silakan klik salah satu folder di panel navigasi kiri untuk melihat isinya.')).toBeVisible();
  });
